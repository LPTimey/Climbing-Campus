...
Mit diesem Wissen haben wir ein @nodejs:long:cap\-Skript geschrieben, welches diese Daten unter Input eines validen @sessiontoken\s aus der Datenbank extrahiert und als @json\-Datei exportiert. Somit konnten wir die Stundenpläne in einem maschinenlesbaren Format erhalten, um diese später für unsere Visualisierung zu nutzen.
...
Schließlich skizzierten wir in @figma ein grobes Storyboard, welches den Handlungsverlauf innerhalb unserer @spa definierten und einen ersten visuellen Eindruck unserer Stildefinition veranschaulichen sollte.
...
Diese folgen genauer einem @hard_surf\-Ansatz mit klar definierten Kanten und präzisen, technischen Formen, um den stark geometrischen Charakter noch einmal zusätzlich zu verstärken.
...
Als Grundlage für die spätere Visualisierung benötigten wir zunächst die entsprechenden Daten. Dafür mussten wir die Umfrage zunächst auf einer geeigneten Plattform umsetzen. Wie bereits einleitend erwähnt, fiel unsere Wahl auf @tally:cap. Dort mussten wir nun unser Umfragelayout in einen funktionierenden Ablauf umbauen, um schließlich eine finale Umfrage zu erhalten, die wir zur Beantwortung an Studierende und Angestellte verteilen können.

Nach diesem Übertragen der Frageblöcke auf @tally:cap haben zunächst eine Pilot-Durchführung mit einer Person durchgeführt. Diese wies uns auf einige Unschönheiten und Fehler innerhalb des Aufbaus hin, sodass wir diese beheben konnten, bevor wir die Umfrage in einem größeren Rahmen ausspielten.


Anschließend begann die eigentliche Verteilung der Umfrage. Zunächst verschickten wir sie über die uns zugänglichen Verteiler, insbesondere verschiedene WhatsApp-Gruppen. Dabei baten wir die Teilnehmenden, die Umfrage nach dem Ausfüllen im Sinne eines Schneeball-Systems weiterzuleiten. In der Praxis erwies sich dieses Konzept jedoch als Wunschdenken, da wir nach etwa zwei Wochen lediglich sieben Teilnehmende erreicht hatten. Daher passten wir unsere Strategie an und sprachen gezielt einzelne Personen direkt an, um sie für die Teilnahme zu gewinnen. Dies geschah sowohl bei Studierenden als auch bei Angestellten der THI.
Nachdem die Ergebnisse so letztendlich erhoben wurden, ließen sich diese ganz einfach als @csv Datei ausgeben, was uns die anschließende Auswertung per @excel:cap sehr vereinfachte.
...
Unter Einschluss der bereits bestehenden konzeptionellen Grundlage konnten wir jetzt schließlich mit der konkreten technischen Ausarbeitung beginnen. Hierzu legten wir zunächst ein @github\-Repository an, um ein zentralisiertes Speichersystem sowie eine Historie an Änderungen gut verwalten zu können. Der nächste Schritt war nun das initiale technische Layouting der @html\-Datei. Wir überlegten uns also konkret, welche Sektionen die finale Ausarbeitung umfassen sollte und legten diese im Anschluss an.
Da wir mit einem 3d-Framework (@threejs) arbeiteten, mussten wir dieses zusätzlich richtig einbinden und einige Vorbereitungen in diesem Kontext anstellen. Dies umfasst vor allem das Aufsetzen eines Animations-Systems, welches wir selbst geschrieben haben. Dieses wurde benötigt, um den einzelnen Objekte Abläufe zuzuschreiben, d.h. jeweils eine Eingangsanimation wenn das Objekt erscheint, eine Idle-Animation beim bloßen Verweilen des Objekts und eine Exit-Animation sobald es wieder verschwindet. Diese lassen sich auch je nach dem, in welcher Sektion sie sich befinden abwandeln. Somit hatten wir volle Kontrolle über alle visuellen Geschehnisse, die beim Scrollen über die Website aufkommen.
...
Aber nicht nur das, dass wäre ja in der Entwicklung sofort aufgefallen. Das zweite Level an Tücke wurde durch den Browser hinzugefügt, da er das Problem für uns (und von uns) versteckte. Genauer kam der Gleitkommazahl-Rundungsfehler nur bei einem Reload --- dem vollen neu laden einer Webseite meist mit "STRG"+"SHIFT"+R --- auf und wurde mit Refreshes --- dem normalen neuladen, bei einmaligem drücken des Neuladenknopfes oder durch Entwicklungsumgebungen --- magisch behoben.
Unsere beste Theorie, ist das der Browser bei Reloads die Positionen der Sections von neuem ausrechnet und bei Reloads von einem Cache weiter rechnet. Dabei würde ein winziger Fehlerwert ($lt.eq 0.01$px), welcher aufgrund seiner Größe von unter einem Pixel nie für Probleme sorgen würde, außer man nutzt unsaubere Gleitkommazahlen vergleiche und springt mit Scroll Snap immer an die gleiche Stelle.

=== Gestaltung der Access Points

Nachdem wir eine rein digitale Informationsvisualisierung entwickelten, stießen wir automatisch auf die Frage, wie man für den Betrachter sichtbar wird. Hierbei entschieden wir als Lösung für diese Problematik, eine Vielzahl an Stickern als Access Point zu gestalten. Diese zeigen eine leicht provokative, und folglich anregende Headline sowie einen kleinen Teaser-Text mit passendem 3d-Objekt. Zusätzlich, und von zentraler Wichtigkeit, befindet sich ganz links ein QR-Code, welcher auf unsere Website verlinken würde.
...
Zum Abschluss haben wir eine Single-Page-Website entwickelt, die mithilfe von @threejs 3D-Modelle zur Datenvisualisierung der Treppen und Stundenpläne an der THI integriert.
